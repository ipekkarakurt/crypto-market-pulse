...
class TradeControllerTest {
    ...
    void createReturnsCreatedWithLocation() throws Exception {
        ...
        mockMvc.perform(post("/trades")
                ...
                .andExpect(jsonPath("$.tradeTime").exists());
    }
    ...
    @Test
    void findRecentRequiresSymbol() throws Exception {
        mockMvc.perform(get("/trades"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Missing required parameter 'symbol'"));
    }

    @Test
    void findRecentReturnsTrades() throws Exception {
        when(tradeService.findRecent("BTC-USD", 100)).thenReturn(List.of(sampleTrade(2L, "BTC-USD")));

        mockMvc.perform(get("/trades").param("symbol", "BTC-USD").param("limit", "100"))
    ...
    void findRecentRejectsInvalidLimit() throws Exception {
        mockMvc.perform(get("/trades").param("symbol", "BTC-USD").param("limit", "0"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Validation failed"))
